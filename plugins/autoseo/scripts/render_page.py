#!/usr/bin/env python3
"""
Shared headless renderer for autoseo.
...
Why
===
Raw HTML produces false negatives on SPAs (an empty ``<div id="root">``,
client-injected schema, or no article text in the server response). This
module provides one shared rendering subsystem for every relevant workflow.
...
- ``never``  : raw HTML only, equivalent to ``fetch_page.py``.
...
from file_safety import write_text_safely  # noqa: E402
...
USER_AGENT = (
    ...
    "(KHTML, like Gecko) Chrome/150.0.7871.115 Safari/537.36 AutoSEO/0.1"
)
...
def _cli() -> None:
    parser = argparse.ArgumentParser(
        description="autoseo shared headless renderer (Playwright + trafilatura)"
    )
