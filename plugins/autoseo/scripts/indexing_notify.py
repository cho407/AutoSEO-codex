...
def _bounded_delay(value: str) -> float:
    delay = float(value)
    if not 0 <= delay <= 10:
        raise argparse.ArgumentTypeError("delay must be between 0 and 10 seconds")
    return delay
...
def _build_indexing_service():
    ...
    if build is None:
        print(
            "Error: google-api-python-client is required; run AutoSEO setup.",
            file=sys.stderr,
        )
        return None
...
def notify_url(
    ...
    *,
    ...
    if not confirmed:
        result["error"] = (
            "external write refused: obtain explicit user confirmation and "
            "retry with --confirm-submit"
        )
        return result
    if action not in {"URL_UPDATED", "URL_DELETED"}:
        result["error"] = "action must be URL_UPDATED or URL_DELETED"
        return result
...
def batch_notify(
    ...
    *,
    ...
    for i, url in enumerate(urls):
        ...
        notification = notify_url(url, action, confirmed=True)
...
def main() -> int:
    ...
    parser.add_argument(
        "--confirm-submit",
        action="store_true",
        help="Confirm the previewed Google Indexing API external write",
    )
    ...
    if args.status:
        ...
        try:
            raw_urls = read_text_limited(args.batch, max_bytes=1024 * 1024)
            urls = [line.strip() for line in raw_urls.splitlines() if line.strip()]
        except (OSError, ValueError) as e:
            print(f"Error reading batch file: {e}", file=sys.stderr)
            return 1
        result = batch_notify(
            urls, args.action, delay=args.delay, confirmed=args.confirm_submit
        )
    elif args.url:
        print(SCOPE_WARNING, file=sys.stderr)
        result = notify_url(args.url, args.action, confirmed=args.confirm_submit)
    else:
        parser.print_help()
        return 1
