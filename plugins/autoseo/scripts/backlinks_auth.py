#!/usr/bin/env python3
"""
Backlink API credential management for AutoSEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/autoseo/backlinks-api.json")
CACHE_DIR = os.path.expanduser("~/.cache/autoseo/commoncrawl")
...
def _chmod_quiet(path: str, mode: int = 0o600) -> None:
    try:
        os.chmod(path, mode)
    except OSError:
        pass
...
def _safe_cache_dir(raw: str) -> str:
    ...
    try:
        ...
        home = None
    if candidate == Path(candidate.anchor).resolve() or candidate == home:
        raise ValueError("Common Crawl cache must be a dedicated subdirectory")
    if not any(candidate == root or candidate.is_relative_to(root) for root in roots):
        raise ValueError(
            "Common Crawl cache must stay within the working, home, or temporary directory"
        )
...
def load_config() -> dict:
    ...
    Reads ~/.config/autoseo/backlinks-api.json first. Any missing fields
    ...
    try:
        config["commoncrawl_cache_dir"] = _safe_cache_dir(
            config.get("commoncrawl_cache_dir", CACHE_DIR)
        )
    except ValueError as exc:
        print(f"Warning: {exc}; using the default cache", file=sys.stderr)
        config["commoncrawl_cache_dir"] = _safe_cache_dir(CACHE_DIR)
...
def detect_tier() -> dict:
    ...
    if has_moz and has_bing:
        return {
            ...
            "missing": "Connect an authorized DataForSEO provider for premium backlink data (paid)",
        }
    elif has_moz:
        return {
            "tier": 1,
...
def get_cache_dir() -> str:
    ...
    config = load_config()
    cache_dir = _safe_cache_dir(config.get("commoncrawl_cache_dir", CACHE_DIR))
...
Free backlink data sources for AutoSEO. No payment required for any of these.
...
-------------------------------------------------
  ...
  5. Copy your API credentials. AutoSEO accepts either:
...
PREMIUM: DATAFORSEO CONNECTOR (paid, most comprehensive)
---------------------------------------------------------
  For commercial-grade backlink data, connect a DataForSEO provider that you
  already trust and authorize. AutoSEO does not install third-party connectors.

  Provides (subject to the provider's current plan): broad link coverage,
            fresh updates, risk signals,
...
def main():
    parser = argparse.ArgumentParser(
        description="Backlink API credential management for AutoSEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
