...
Consumes JSON output from autoseo-google scripts and generates formatted reports
...
class _EscapedHTML(str):
    """Marker used to make repeated boundary escaping idempotent."""


def escape(value, quote=True):
    if isinstance(value, _EscapedHTML):
        return value
    return _EscapedHTML(_stdlib_escape(str(value), quote=quote))


def _escape_html_tree(value):
    """Escape every string value while preserving mapping keys and types."""
    if isinstance(value, str):
        return escape(value)
    if isinstance(value, dict):
        return {key: _escape_html_tree(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_escape_html_tree(item) for item in value]
    if isinstance(value, tuple):
        return tuple(_escape_html_tree(item) for item in value)
    return value


def _css_string(value: object) -> str:
    """Escape an untrusted value for a double-quoted CSS string."""
    return (
        str(value)
        .replace("\\", "\\\\")
        .replace("<", "\\3c ")
...
def _resolve_report_dir(value: object) -> Path:
    ...
    for root in allowed:
        try:
            ...
            continue
    raise ValueError("report output must be within the current directory or home directory")
...
def _build_css(domain: str) -> str:
    """
    Professional A4 report CSS for AutoSEO's generated reports.
...
def _img_tag(path, alt="Chart"):
    ...
    return f'<img src="{escape(uri)}" style="max-width: 100%;" alt="{escape(alt)}">'
...
def _chart_html(path, caption, fig_num, alt="Chart"):
    ...
    return (
        ...
        f'      <img src="{escape(uri)}" style="width: 85%;" alt="{escape(alt)}">\n'
        f'      <div class="chart-caption">Figure {escape(fig_num)}: {escape(caption)}</div>\n'
        f'    </div>\n'
    )
...
def _metric_card(value, label, color=None):
    ...
    safe_color = color if isinstance(color, str) and re.fullmatch(r"#[0-9A-Fa-f]{6}", color) else None
    style = f' style="color: {safe_color};"' if safe_color else ""
    return (
...
def _build_title_page(domain, report_title, subtitle, score=None, score_label=None, meta_items=None):
    ...
    if score is not None:
        score_html = (
            ...
            f'    <div class="score-number">{escape(score)}'
            f'<span style="font-size: 20pt; color: #94a3b8;">/100</span></div>\n'
            f'    <div class="score-label">{escape(score_label or "Lighthouse Performance Score")}</div>\n'
            f'  </div>\n'
        )

    meta_html = ""
    if meta_items:
        spans = "\n    ".join(f"<span>{escape(item)}</span>" for item in meta_items)
        meta_html = (
            f'  <div class="meta">\n'
            f'    {spans}\n'
            f'  </div>\n'
    ...
    if google_logo_path.exists():
        google_logo_html = (
            ...
            f'    <img src="{escape(google_logo_path.resolve().as_uri())}" style="height: 20px; opacity: 0.7;" alt="Google">\n'
    ...
    return (
        ...
        f'  <div class="badge">{escape(report_title)}</div>\n'
        f'  <h1>{escape(domain)}</h1>\n'
        f'  <div class="subtitle">Prepared by AutoSEO</div>\n'
        f'{score_html}'
        f'{meta_html}'
        f'{google_logo_html}'
        f'</div>\n'
...
def _build_toc(sections_info):
    ...
    for sec in sections_info:
        ...
        items.append(
            ...
            f'<span>{escape(sec["num"])}. {escape(sec["title"])}</span>{score_html}'
            f'</li>'
        )
        for sub in sec.get("subs", []):
            items.append(f'    <li class="toc-sub"><span>{escape(sub)}</span></li>')
...
def _build_methodology_footer(domain, timestamp, gsc_warning=""):
    ...
    return (
        ...
        f'    Report generated by AutoSEO &mdash; Google SEO Intelligence Skill &mdash; '
        f'{timestamp}<br>\n'
...
def generate_report(
    ...
    *,
    ...
    """
    if report_type not in {"cwv-audit", "gsc-performance", "indexation", "full"}:
        return {"report_type": report_type, "domain": domain, "files": [],
                "error": "unsupported report type"}
    if output_format not in {"pdf", "html", "xlsx", "both", "all"}:
        return {"report_type": report_type, "domain": domain, "files": [],
                "error": "unsupported output format"}
    if not isinstance(data, dict):
        return {"report_type": report_type, "domain": domain, "files": [],
                "error": "report data must be a JSON object"}
    raw_data = data
    raw_domain = str(domain)
    safe_domain = _safe_filename_component(raw_domain)
    base_name = f"Google-SEO-Report-{safe_domain}-{report_type}"
    try:
        output_dir = _resolve_report_dir(output_dir)
    except ValueError as exc:
        return {"report_type": report_type, "domain": raw_domain, "files": [],
                "error": str(exc)}
    planned_paths = []
    if output_format in ("html", "both", "all"):
        planned_paths.append(output_dir / f"{base_name}.html")
    if output_format in ("pdf", "both", "all"):
        planned_paths.append(output_dir / f"{base_name}.pdf")
    if output_format in ("xlsx", "all"):
        planned_paths.append(output_dir / f"{base_name}.xlsx")
    if not overwrite:
        conflict = next((path for path in planned_paths if path.exists()), None)
        if conflict:
            return {"report_type": report_type, "domain": raw_domain, "files": [],
                    "error": f"output already exists: {conflict}; pass --overwrite to replace it"}
    charts_dir = output_dir / "charts"
    chart_names = {
        "lighthouse_gauges.png",
        "cwv_distributions.png",
        "cwv_timeline.png",
        "top_queries.png",
        "index_status.png",
    }
    if not overwrite and charts_dir.is_dir():
        conflict = next(
            (path for path in charts_dir.iterdir() if path.name in chart_names),
            None,
        )
        if conflict:
            return {"report_type": report_type, "domain": raw_domain, "files": [],
    ...
    result = {"report_type": report_type, "domain": raw_domain, "files": [], "error": None}
    ...
    # Escape every external string once before it reaches an HTML section.
    # Chart generation above intentionally uses the original values.
    data = _escape_html_tree(raw_data)
    domain = escape(raw_domain)
...
def generate_xlsx(
    ...
    *,
    ...
    # Neutralize spreadsheet formulas from untrusted API/site strings.
        ...
        for row in sheet.iter_rows():
            for cell in row:
                if not isinstance(cell.value, str):
                    continue
                value = illegal_control_re.sub("", cell.value)
    ...
    if filename is None:
        filename = (
            f"Google-SEO-Report-{_safe_filename_component(domain)}-"
            f"{datetime.now().strftime('%Y%m%d-%H%M')}.xlsx"
        )
    filename_path = Path(filename)
    if filename_path.name != filename or filename_path.suffix.lower() != ".xlsx":
        raise ValueError("xlsx filename must be a plain .xlsx basename")
...
def main():
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Replace existing report artifacts explicitly"
    )
    ...
    if args.data:
        ...
        try:
            ...
            if len(raw.encode("utf-8")) > max_json_bytes:
                raise ValueError("stdin JSON exceeds the 50 MiB safety limit")
            data = json.loads(raw)
