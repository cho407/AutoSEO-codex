...
Approach
========
1. WHOIS lookup through a bounded socket query against the IANA referral
   chain. Every destination is DNS-resolved, checked as public, and pinned
   before connecting.
...
delegated to the ``autoseo-content`` skill which orchestrates it.
...
from url_safety import is_safe_ip, normalize_hostname  # noqa: E402
...
def _normalize_domain(domain: str) -> str:
    """Return a conservative ASCII DNS name suitable for a WHOIS query."""
    ...
    try:
        ...
        raise ValueError("domain must be a DNS name, not an IP address")
    if len(value) > 253:
...
def _query_whois(host: str, domain: str) -> str:
    """Query a public WHOIS server with DNS pinning and a response cap."""
    ...
    if not addresses or any(not is_safe_ip(address) for address in addresses):
        raise OSError(f"WHOIS server {normalized_host!r} is not public")

    with socket.create_connection((addresses[0], 43), timeout=10) as sock:
        sock.sendall(f"{domain}\r\n".encode("ascii"))
        chunks: list[bytes] = []
        total = 0
        while True:
            chunk = sock.recv(min(4096, MAX_WHOIS_BYTES + 1 - total))
            if not chunk:
                break
            total += len(chunk)
            if total > MAX_WHOIS_BYTES:
                raise OSError("WHOIS response exceeded the 1 MiB safety limit")
            chunks.append(chunk)
...
def _socket_whois(domain: str) -> Optional[str]:
    ...
    try:
        ...
        iana_text = _query_whois("whois.iana.org", domain)
    ...
    try:
        return _query_whois(referral, domain)
    except (OSError, ValueError, socket.gaierror):
        return None
...
def lookup(domain: str) -> dict:
    try:
        ...
        return {
            ...
            "notes": [f"invalid domain: {exc}"],
        }
    raw = _socket_whois(domain)
    source = "iana-referral"
    if raw is None:
        return {
            "domain": domain,
            "whois_source": None,
            ...
            "registrar": None,
            "years_registered": None,
            "notes": ["whois unavailable — check DNS and egress on TCP/43"],
        }
