#!/usr/bin/env python3
"""
Google API credential management for AutoSEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/autoseo/google-api.json")
TOKEN_PATH = os.path.expanduser("~/.config/autoseo/oauth-token.json")
...
_OAUTH_TOKEN_URIS = frozenset(
    {
        "https://oauth2.googleapis.com/token",
        "https://accounts.google.com/o/oauth2/token",
    }
)
_OAUTH_AUTH_URIS = frozenset(
    {
...
def load_config() -> dict:
    ...
    Reads ~/.config/autoseo/google-api.json first. Any missing fields
...
def _load_oauth_client(creds_path: str) -> Optional[dict]:
    ...
    try:
        _chmod_quiet(creds_path, 0o600)
        with open(creds_path, "r") as f:
            data = json.load(f)
        client = data.get("web", data.get("installed", {}))
        if not isinstance(client, dict):
            raise ValueError("OAuth client entry must be an object")
        _oauth_endpoint(client, "token_uri")
        _oauth_endpoint(client, "auth_uri")
        if not client.get("client_id") or not client.get("client_secret"):
            raise ValueError("OAuth client is missing client_id or client_secret")
        return client
...
def _oauth_endpoint(client: dict, field: str) -> str:
    """Return a known Google OAuth endpoint; refuse credential exfiltration."""
    if field == "token_uri":
        default = "https://oauth2.googleapis.com/token"
        allowed = _OAUTH_TOKEN_URIS
    elif field == "auth_uri":
        default = "https://accounts.google.com/o/oauth2/auth"
        allowed = _OAUTH_AUTH_URIS
    else:  # pragma: no cover - internal programming error
        raise ValueError(f"unsupported OAuth endpoint field: {field}")
    value = str(client.get(field) or default).rstrip("/")
    ...
    if (
        ...
        raise ValueError(f"untrusted Google OAuth {field}: {value!r}")
    return value


def _read_oauth_json(response) -> dict:
    raw = response.read(_MAX_OAUTH_RESPONSE_BYTES + 1)
    if len(raw) > _MAX_OAUTH_RESPONSE_BYTES:
        raise ValueError("OAuth response exceeded the 1 MiB safety limit")
...
def _chmod_quiet(path: str, mode: int) -> None:
    ...
    support POSIX permissions). Used to remediate older token files that may
    have inherited a permissive umask."""
    try:
        os.chmod(path, mode)
    except OSError:
        pass
...
def _load_oauth_token() -> Optional[dict]:
    ...
    Also remediates older file permissions that may have inherited the umask
    default (typically 0o644). Each load forces the file to 0o600.
    """
...
def _save_oauth_token(token_data: dict):
    ...
    Belt-and-suspenders sequence:
        1. Pre-chmod any existing file that may have inherited umask=022.
    ...
    os.makedirs(token_dir, exist_ok=True, mode=0o700)
    _chmod_quiet(token_dir, 0o700)
...
def _refresh_oauth_token(client: dict, token_data: dict) -> Optional[dict]:
    ...
    try:
        req = urllib.request.Request(_oauth_endpoint(client, "token_uri"), data=params)
        with urllib.request.urlopen(req) as resp:
            new_data = _read_oauth_json(resp)
...
def run_oauth_flow(creds_path: str):
    ...
    auth_url = (
        f"{_oauth_endpoint(client, 'auth_uri')}"
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if (
                code
                and len(code) <= 4096
                and secrets.compare_digest(state, oauth_state)
            ):
                auth_code[0] = code
...
def _exchange_code(client: dict, code: str, creds_path: Optional[str] = None):
    ...
    try:
        req = urllib.request.Request(
            _oauth_endpoint(client, "token_uri"), data=params
        )
        with urllib.request.urlopen(req) as resp:
            token_data = _read_oauth_json(resp)
...
6. CREATE CONFIG FILE
   mkdir -p ~/.config/autoseo
   Save to ~/.config/autoseo/google-api.json:
...
def main():
    parser = argparse.ArgumentParser(
        description="Google API credential management for AutoSEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
