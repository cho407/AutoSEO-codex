...
Config: ~/.config/autoseo/dataforseo-costs.json
Ledger: ~/.config/autoseo/dataforseo-ledger.json
...
Security fixes: config path corrected to ~/.config/autoseo/
"""
...
# ----- paths -----
CONFIG_DIR = Path.home() / ".config" / "autoseo"
...
def _load_ledger():
    ...
    if not LEDGER_FILE.exists():
        return {"entries": []}
    _chmod_quiet(LEDGER_FILE, 0o600)
    if fcntl:
        lock_path = LEDGER_FILE.with_suffix(".lock")
        with open(lock_path, "w") as lock_file:
            _chmod_quiet(lock_path, 0o600)
...
def _save_ledger(ledger):
    ...
    if fcntl:
        ...
        with open(lock_path, "w") as lock_file:
            ...
            try:
                _write_private_json(LEDGER_FILE, ledger)
            finally:
                fcntl.flock(lock_file, fcntl.LOCK_UN)
    else:
        _write_private_json(LEDGER_FILE, ledger)
...
def cmd_check(args):
    ...
    # Every paid call requires immediate user confirmation. Configuration can
    # strengthen the warning, but cannot waive that confirmation.
    needs_approval = True
    approval_reason = "explicit_confirmation_required"
...
def main():
    ...
    p_cfg.add_argument("--threshold", type=_non_negative_float)
    p_cfg.add_argument("--daily-limit", type=_non_negative_float, dest="daily_limit")
