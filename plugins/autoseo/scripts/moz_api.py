#!/usr/bin/env python3
"""
Moz Link Explorer API client for AutoSEO.
...
RATE_LIMIT_DELAY = 10
RATE_LIMIT_FILE = os.path.expanduser("~/.cache/autoseo/moz_last_request.lock")
MAX_MOZ_RESPONSE_BYTES = 50 * 1024 * 1024
...
def _moz_request(path: str, body: dict, api_key: str) -> dict:
    ...
    try:
        ...
        read_limited_response(response, max_bytes=MAX_MOZ_RESPONSE_BYTES)
        ...
        return {
            "status": "error",
            "data": None,
            "error": f"Moz request failed ({type(e).__name__})",
            "metadata": {"source": "moz"},
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Moz Link Explorer API client for AutoSEO"
    )
    parser.add_argument(
        "command",
