...
preload sequence". This makes the report materially more actionable than an
overall LCP value alone.
...
MAX_CRUX_RESPONSE_BYTES = 20 * 1024 * 1024
...
def _read_bounded(response) -> bytes:
    raw = response.read(MAX_CRUX_RESPONSE_BYTES + 1)
    if len(raw) > MAX_CRUX_RESPONSE_BYTES:
        raise ValueError("CrUX response exceeded the 20 MiB safety limit")
    return raw
