...
from url_safety import read_limited_response  # noqa: E402
...
def _bounded_keyword(value: str) -> str:
    ...
    if not value or len(value) > 500:
        raise argparse.ArgumentTypeError("keyword must contain 1-500 characters")
    return value
...
def _bounded_depth(value: str) -> int:
    number = int(value)
    if not 1 <= number <= 100:
        raise argparse.ArgumentTypeError("depth must be between 1 and 100")
    return number


def _require_cost_confirmation(args, endpoint_keys: list[str]) -> bool:
    if args.confirm_cost:
        return True
    endpoints = [COST_ENDPOINTS[key] for key in endpoint_keys]
    json.dump(
        {
            "status": "blocked",
            ...
            "endpoints": endpoints,
            "message": (
                "Run dataforseo_costs.py check for the listed endpoint(s), show "
                "the estimate to the user, then retry with --confirm-cost only "
...
def _post_task(
    ...
    try:
        resp = requests.post(
            url, json=payload, headers=headers, timeout=30, verify=True, stream=True
        )
        read_limited_response(resp, max_bytes=MAX_PROVIDER_RESPONSE_BYTES)
        resp.raise_for_status()
        data = resp.json()
    except requests.exceptions.RequestException as exc:
        return {"error": "transport_error", "message": type(exc).__name__}
    except (ValueError, json.JSONDecodeError) as exc:
        return {"error": "invalid_response", "message": type(exc).__name__}
...
def _extract_task_id(response: dict[str, Any]) -> Optional[str]:
    ...
    if tasks and "id" in tasks[0]:
        ...
        if re.fullmatch(r"[A-Za-z0-9_-]{1,200}", task_id):
            return task_id
    return None
...
def cmd_search(args):
    ...
    endpoint_key = (
        "google_products" if args.marketplace == "google" else "amazon_products"
    )
    if not _require_cost_confirmation(args, [endpoint_key]):
        return
...
def cmd_sellers(args):
    ...
    if not _require_cost_confirmation(args, ["google_sellers"]):
        return
    username, password = _get_credentials()
    headers = _auth_header(username, password)
...
def cmd_compare(args):
    ...
    if not _require_cost_confirmation(args, ["google_products", "amazon_products"]):
        return
    username, password = _get_credentials()
    headers = _auth_header(username, password)
...
def main() -> int:
    ...
    # Shared arguments
    def add_common(p):
        p.add_argument("keyword", type=_bounded_keyword, help="Product search keyword")
        p.add_argument(
        ...
        p.add_argument(
            "--confirm-cost",
            action="store_true",
            help="Confirm the displayed DataForSEO cost estimate",
        )
    ...
    p_search.add_argument("--depth", type=_bounded_depth, default=100, help="Number of results")
    ...
    p_compare.add_argument("--depth", type=_bounded_depth, default=100, help="Number of results")
    ...
    endpoint_keys = {
        ...
        "sellers": ["google_sellers"],
        "compare": ["google_products", "amazon_products"],
    }[args.command]
    if not _require_cost_confirmation(args, endpoint_keys):
        return 2
