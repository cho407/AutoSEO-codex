#!/usr/bin/env python3
"""
Backlink verification crawler for AutoSEO.
...
def _head_check(url: str, timeout: int = 15) -> dict:
    ...
    try:
        resp = safe_requests_head(
            ...
            headers={"User-Agent": "AutoSEO/0.1 BacklinkVerifier"},
        )
        return {
...
def main():
    parser = argparse.ArgumentParser(
        description="Backlink verification crawler for AutoSEO"
    )
    parser.add_argument(
        "--target",
        required=True,
    ...
    try:
        if args.links == "-":
            links = json.loads(read_stream_limited(sys.stdin))
        else:
            links = json.loads(
                read_text_limited(args.links, extensions={".json"})
            )
