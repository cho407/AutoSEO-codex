#!/usr/bin/env python3
"""
Canonical URL safety module for autoseo.
...
validate_url(url) -> bool
    ...
    ranges. Does NOT resolve DNS. This compatibility check is used by
    google_auth.py.
...
autoseo scripts are intentionally single-threaded; parallelism is
...
def read_limited_response(
    ...
    *,
    ...
    """Read a streamed response into memory with a decompressed-size cap.

    ``requests`` transparently decompresses chunks, so the limit protects
    against both oversized bodies and compressed response bombs. The filled
    response keeps normal ``.content`` and ``.text`` behavior for callers.
    """
    if max_bytes <= 0:
        response.close()
        raise URLSafetyError("max response size must be positive")
    ...
    if content_length:
        ...
        if declared_length is not None and declared_length > max_bytes:
            ...
            raise URLSafetyError(
                f"response body exceeds {max_bytes} byte safety limit"
            )
...
def _reject_authority_confusion(url: str, parsed) -> None:
    ...
    private host. autoseo never needs credentials in audit URLs, so
...
# This is a deliberate choice: autoseo scripts run one URL fetch at a
...
def _pin_dns(hostname: str, pinned_ip: str, port: int) -> Iterator[None]:
    ...
    Fall-through validation protects redirect targets from DNS rebinding:
    ``requests.Session.get(allow_redirects=True)`` may follow
    ...
    if not _dns_patch_lock.acquire(blocking=False):
        raise URLSafetyError(
            ...
            "autoseo url_safety is not thread-safe by design."
        )
...
def safe_requests_get(
    ...
    **kwargs,
    ...
    with _pin_dns(parsed.hostname, pinned_ip, port):
        response = requests.get(norm_url, timeout=timeout, **kwargs)
        return read_limited_response(response, max_bytes=max_response_bytes)
...
def safe_requests_head(
    ...
    **kwargs,
    ...
    kwargs["stream"] = True
    with _pin_dns(parsed.hostname, pinned_ip, port):
        response = requests.head(norm_url, timeout=timeout, **kwargs)
        return read_limited_response(response, max_bytes=max_response_bytes)
...
def _cli() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Validate a URL against autoseo's SSRF policy."
    )
