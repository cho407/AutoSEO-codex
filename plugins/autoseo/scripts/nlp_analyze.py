...
try:
    ...
    from url_safety import URLSafetyError, read_limited_response, safe_requests_get
except ImportError:
    import os
...
MAX_NLP_RESPONSE_BYTES = 20 * 1024 * 1024
...
def analyze_text(
    ...
    *,
    ...
    if not confirmed_cost:
        result["error"] = (
            "billable API request refused: review current Google Cloud Natural "
            "Language pricing and retry with --confirm-cost after explicit approval"
        )
        return result
    ...
    if wants_entities:
        ...
        try:
            ...
            read_limited_response(resp, max_bytes=MAX_NLP_RESPONSE_BYTES)
        ...
        except (requests.exceptions.RequestException, URLSafetyError, ValueError) as e:
    ...
    *,
    ...
    try:
        resp = safe_requests_get(
            url,
            timeout=30,
            headers={"User-Agent": "Mozilla/5.0 (compatible; AutoSEO/0.1 NLP Analyzer)"},
        )
        resp.raise_for_status()
        html = resp.text
...
def main() -> int:
    ...
    parser.add_argument(
        "--confirm-cost",
        action="store_true",
        help="Confirm a potentially billable Google Cloud NLP request",
    )
