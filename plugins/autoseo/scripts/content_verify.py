...
lack a nearby citation marker. Designed for the ``@autoseo content verify``
workflow: a fact-check pass on drafts before publish.
...
def main() -> int:
    ...
    try:
        text = (
            read_stream_limited(sys.stdin)
            if args.source == "-"
            else read_text_limited(args.source)
        )
