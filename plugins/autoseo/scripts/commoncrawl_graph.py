#!/usr/bin/env python3
"""
Common Crawl Web Graph parser for AutoSEO.
...
_GRAPH_SUFFIXES = frozenset({VERTICES_SUFFIX, EDGES_SUFFIX, RANKINGS_SUFFIX})
...
def _validate_release(release: str) -> str:
    ...
    if not _RELEASE_RE.fullmatch(value):
        raise ValueError("invalid Common Crawl release name")
    return value
...
def _normalize_domain(domain: str) -> str:
    ...
    if "://" in value:
        if not validate_url(value):
            raise ValueError("invalid or blocked domain URL")
    ...
    try:
        ...
        raise ValueError("Common Crawl graph lookup requires a DNS name, not an IP")
    labels = value.split(".")
    if (
        len(labels) < 2
        or len(value) > 253
...
def _graph_file_url(release: str, suffix: str) -> str:
    ...
    if suffix not in _GRAPH_SUFFIXES:
        raise ValueError("invalid Common Crawl graph suffix")
...
def _get_latest_release() -> Optional[str]:
    ...
    for release in KNOWN_RELEASES:
        ...
        try:
            resp = safe_requests_head(url, timeout=10, allow_redirects=True)
...
def _get_cache_path(domain: str, release: str, data_type: str) -> str:
    ...
    safe_release = _validate_release(release)
    if data_type != "combined":
        raise ValueError("invalid Common Crawl cache data type")
    return os.path.join(cache_dir, f"{safe_domain}-{safe_release}-{data_type}.json")
...
def _stream_gz_chunked(url: str, target_domain: str, timeout: int = 120,
    ...
    max_decompressed_bytes = 1024 * 1024 * 1024  # 1 GiB processing cap
            ...
            for chunk in resp.iter_content(chunk_size=256 * 1024):
                ...
                while pending:
                    ...
                    if total_decompressed > max_decompressed_bytes:
                        return matches
                    text = leftover + decoded.decode("utf-8", errors="replace")
                    lines = text.split("\n")
                    leftover = lines[-1]
                    if len(leftover) > max_line_chars:
                        return matches
                    ...
                    for line in lines[:-1]:
                        ...
                        if any(
                            field == target_domain
                            or field.endswith(f".{target_domain}")
                            for field in fields
                        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Common Crawl Web Graph parser for AutoSEO"
    )
    parser.add_argument(
        "domain",
        nargs="?",
