...
from file_safety import read_text_limited  # noqa: E402
    ...
    *,
    ...
    if not _belongs_to_host(key_location, host):
        return {"ok": False, "error": "keyLocation host does not match declared host",
                "submitted": 0}
    try:
        validate_url_strict(key_location)
    except URLSafetyError as exc:
        return {"ok": False, "error": f"keyLocation refused: {exc}", "submitted": 0}
    ...
    try:
        ...
        read_limited_response(resp, max_bytes=1024 * 1024)
    except (requests.exceptions.RequestException, URLSafetyError) as exc:
...
def _load_urls(arg_urls: list[str], urls_file: str | None) -> list[str]:
    ...
    if urls_file:
        for line in read_text_limited(urls_file, max_bytes=5 * 1024 * 1024).splitlines():
            line = line.strip()
...
def main() -> int:
    ...
    parser.add_argument(
        "--confirm-submit",
        action="store_true",
        help="Confirm the previewed IndexNow external write",
    )
    ...
    if args.json:
        ...
        print(
            f"  Submitted: {result['submitted']} URLs to "
            f"{result.get('endpoint', INDEXNOW_ENDPOINT)}"
        )
