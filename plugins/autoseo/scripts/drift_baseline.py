...
Storage: ~/.cache/autoseo/drift/baselines.db
"""
...
DB_DIR = os.path.expanduser("~/.cache/autoseo/drift")
...
def fetch_page_data(url: str) -> dict:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            encoding="utf-8",
            suffix=".html",
            prefix="autoseo-drift-",
            delete=False,
        ) as tmp:
            tmp_path = tmp.name

        proc = subprocess.run(
            [sys.executable, fetch_script, url, "--output", tmp_path, "--overwrite"],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
