...
|---------|---------|-------------|
| `@autoseo ecommerce <url>` | Full e-commerce SEO analysis of a product page or store | Optional |
| `@autoseo ecommerce products <keyword>` | Google Shopping competitive analysis | Required |
| `@autoseo ecommerce gaps <domain>` | Keyword gap: organic vs Shopping visibility | Required |
| `@autoseo ecommerce schema <url>` | Product schema validation and enhancement | No |
...
```
1. <plugin-root>/scripts/autoseo run render_page.py <url> --mode auto → raw/rendered HTML
2. <plugin-root>/scripts/autoseo run parse_html.py --url <url>   → SEO elements
...
```bash
<plugin-root>/scripts/autoseo run dataforseo_costs.py check merchant_google_products_search
```
...
After each call:
```bash
<plugin-root>/scripts/autoseo run dataforseo_costs.py log merchant_google_products_search <cost>
```
...
<plugin-root>/scripts/autoseo run dataforseo_merchant.py search "<keyword>" --marketplace google

# Seller analysis: merchant ratings and dominance
<plugin-root>/scripts/autoseo run dataforseo_merchant.py sellers "<keyword>"

# Normalize results for analysis
<plugin-root>/scripts/autoseo run dataforseo_normalize.py results.json --module merchant
```
...
```bash
<plugin-root>/scripts/autoseo run dataforseo_costs.py check merchant_amazon_products_search
```
...
<plugin-root>/scripts/autoseo run dataforseo_merchant.py search "<keyword>" --marketplace amazon

# Cross-marketplace comparison
<plugin-root>/scripts/autoseo run dataforseo_merchant.py compare "<keyword>"
```
...
1. Fetch organic rankings via autoseo-dataforseo:
...
|-------|------------------|
| **autoseo-schema** | Delegates Product schema generation; reuses validation logic |
| **autoseo-images** | Product image audit (alt text, format, dimensions), plus `DigitalSourceType: TrainedAlgorithmicMedia` IPTC label for AI-generated product images (Merchant Center requirement) |
| **autoseo-content** | Product description E-E-A-T and uniqueness analysis |
| **autoseo-dataforseo** | Organic keyword rankings for gap analysis |
| **autoseo-technical** | Core Web Vitals for product pages (LCP on hero image) |
| **autoseo-google** | GSC indexation + Performance data for product URLs (NOT Merchant Center feed validation, that is done in Merchant Center / the **Merchant API**; the legacy Content API for Shopping sunsets 2026-08-18) |
...
<plugin-root>/scripts/autoseo run ucp_check.py https://store.example.com --json

# With endpoint reachability probes (HEAD each declared capability)
<plugin-root>/scripts/autoseo run ucp_check.py https://store.example.com --probe-endpoints --json
```
...
| Non-product page | URL is category/homepage | Detect page type, suggest `@autoseo ecommerce schema` instead |
