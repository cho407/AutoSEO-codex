...
| Command | Purpose |
|---------|---------|
| `@autoseo drift baseline <url>` | Capture current SEO state as a "known good" snapshot |
| `@autoseo drift compare <url>` | Compare current page state to stored baseline |
| `@autoseo drift history <url>` | Show change history and past comparisons |
...
```
~/.cache/autoseo/drift/baselines.db
```
...
2. Fetch page via `<plugin-root>/scripts/autoseo run fetch_page.py <URL>`
3. Parse HTML via `<plugin-root>/scripts/autoseo run parse_html.py <URL>`
4. Optionally fetch CWV via `<plugin-root>/scripts/autoseo run pagespeed_check.py <URL>` (use `--skip-cwv` to skip)
5. Hash HTML body and schema content (SHA-256)
6. Store snapshot in SQLite

**Execution:**
```bash
<plugin-root>/scripts/autoseo run drift_baseline.py <url>
<plugin-root>/scripts/autoseo run drift_baseline.py <url> --skip-cwv
```
...
**Execution:**
```bash
<plugin-root>/scripts/autoseo run drift_compare.py <url>
<plugin-root>/scripts/autoseo run drift_compare.py <url> --baseline-id 5
<plugin-root>/scripts/autoseo run drift_compare.py <url> --skip-cwv
```
...
```bash
<plugin-root>/scripts/autoseo run drift_report.py <comparison_json_file> --output drift-report.html
```
...
**Execution:**
```bash
<plugin-root>/scripts/autoseo run drift_history.py <url>
<plugin-root>/scripts/autoseo run drift_history.py <url> --limit 10
```
...
|---------|----------------|
| Schema removed or modified | Run `@autoseo schema <url>` for full validation |
| CWV regression | Run `@autoseo technical <url>` for performance audit |
| Title or meta description changed | Run `@autoseo page <url>` for content analysis |
| Canonical changed or removed | Run `@autoseo technical <url>` for indexability check |
| Noindex added | Run `@autoseo technical <url>` for crawlability audit |
| H1/heading structure changed | Run `@autoseo content <url>` for E-E-A-T review |
| OG tags removed | Run `@autoseo page <url>` for social sharing analysis |
| Status code changed to error | Run `@autoseo technical <url>` for full diagnostics |
...
- **All URL fetching** goes through `<plugin-root>/scripts/autoseo run fetch_page.py`, which enforces SSRF protection
...
```
@autoseo drift baseline https://example.com     # Before deploy
# ... deploy happens ...
@autoseo drift compare https://example.com      # After deploy
```

### Ongoing Monitoring
```
@autoseo drift baseline https://example.com     # Initial capture
# ... weeks later ...
@autoseo drift compare https://example.com      # Check for drift
@autoseo drift history https://example.com      # Review all changes
```

### Investigating a Traffic Drop
```
@autoseo drift compare https://example.com      # What changed?
@autoseo drift history https://example.com      # When did it change?
```
