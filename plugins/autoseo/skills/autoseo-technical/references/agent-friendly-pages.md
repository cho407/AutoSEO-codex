...
CLI `lighthouse <url> --only-categories=agentic-browsing` (Node.js
...
Use a separately installed, reviewed Lighthouse executable; never allow `npx`
to download `@latest` during an audit.
...
`<plugin-root>/scripts/autoseo run render_page.py <URL> --mode auto --a11y-tree --json` loads the
page headlessly and captures Chromium's full accessibility tree through CDP.
Use `<plugin-root>/scripts/autoseo run agent_ux_check.py <URL> --json` for the bounded Agent-UX
