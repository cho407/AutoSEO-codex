...
- XML sitemap: run `<plugin-root>/scripts/autoseo run sitemap_discovery.py <url> --json`; require a
...
| CodexBot | OpenAI | `CodexBot` | Model training |
...
**Recommendation:** Consider your AI visibility strategy before blocking. Being cited by AI systems drives brand awareness and referral traffic. Cross-reference the `autoseo-geo` skill for the full AI crawler/fetcher taxonomy.
...
- See autoseo-schema skill for full analysis
...
<plugin-root>/scripts/autoseo run agent_ux_check.py https://example.com --json
```
...
`<plugin-root>/scripts/autoseo run render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `<plugin-root>/scripts/autoseo run pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `<plugin-root>/scripts/autoseo run crux_history.py <url> --json` for 25-week CWV trends, and `<plugin-root>/scripts/autoseo run gsc_inspect.py <url> --json` for real indexation status per URL.
