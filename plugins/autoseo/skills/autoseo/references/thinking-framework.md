...
This is the canonical methodology autoseo uses to assemble raw findings
...
- What does **Google's primary-source guidance** say about the
  ...
  to Google (see `skills/autoseo-geo/references/google-ai-optimization-guide.md`).
...
- `autoseo-content` thin-content finding × `autoseo-cluster` SERP-overlap data →
  consolidate three weak pages into one cluster hub.
- `autoseo-schema` missing Product schema × `autoseo-ecommerce` UCP-not-declared
  → both close the same agent-era buying gap; bundle as one
  recommendation.
- `autoseo-geo` low AI-citation rate × `autoseo-backlinks` brand-mention
  underweight → mentions matter 3× more than backlinks for AI
  citations; reframe link-building budget into PR / Reddit / YouTube.
- `autoseo-technical` SPA detection × `autoseo-content` missing main-content
...
- If a prior recommendation is now stale because Google's guidance shifted,
...
- A PDF via `<plugin-root>/scripts/autoseo run google_report.py` when the user asks for one.
...
- Capture a baseline via `@autoseo drift baseline <url>` so subsequent
...
- Surface what autoseo itself **could not measure** (offline
...
Every full-site audit (`@autoseo audit`) and deep-page audit (`@autoseo page`)
...
Single-purpose commands (`@autoseo schema`, `@autoseo images`, `@autoseo technical`,
...
These principles are autoseo's; they are not the user's. Surface them
for the user when:
