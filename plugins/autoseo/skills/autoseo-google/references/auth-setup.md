...
3. Name it (e.g., "AutoSEO") and note the project ID
...
2. Name: `autoseo` (or similar)
3. Skip optional permissions steps
4. Click on the created service account > **Keys > Add Key > Create new key > JSON**
5. Download the JSON file and store it securely (e.g., `~/.config/autoseo/service_account.json`)
...
```json
{
  "service_account_path": "~/.config/autoseo/service_account.json",
...
```bash
<plugin-root>/scripts/autoseo run google_auth.py --check
```
