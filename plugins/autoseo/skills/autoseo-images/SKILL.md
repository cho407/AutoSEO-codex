...
`<plugin-root>/scripts/autoseo run parse_html.py` classifies each image's lazy-loading mechanism via the
...
# Batch inject to all images in directory (keeps exiftool _original backups)
exiftool \
...
<plugin-root>/scripts/autoseo run iptc_ai_label.py audit ./images/ --json

# Audit a single image
<plugin-root>/scripts/autoseo run iptc_ai_label.py audit ./hero.webp --json

# Inject the AI label into an image
<plugin-root>/scripts/autoseo run iptc_ai_label.py inject ./ai-hero.webp \
...
These commands modify image metadata. Review the exact files with the audit
command, obtain explicit user confirmation, and keep exiftool's default
`_original` backup. Use `-overwrite_original` only if the user separately asks
to discard that backup.
...
> Statement of Rights). Cross-link `autoseo-schema` for the ImageObject markup.
...
When `@autoseo images optimize` is run on AI-generated assets, prompt the user to
...
in cross-reference with `autoseo-ecommerce`.
