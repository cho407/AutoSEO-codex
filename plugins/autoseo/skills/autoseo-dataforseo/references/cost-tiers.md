...
Configure with: `<plugin-root>/scripts/autoseo run dataforseo_costs.py config --mode threshold --threshold 0.50 --daily-limit 10.00`
...
1. Run `<plugin-root>/scripts/autoseo run dataforseo_costs.py check <endpoint> [--count N]`
...
5. After call completes, log: `<plugin-root>/scripts/autoseo run dataforseo_costs.py log <endpoint> <cost>`
