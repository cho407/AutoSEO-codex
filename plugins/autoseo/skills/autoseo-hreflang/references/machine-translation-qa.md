...
## Signals the autoseo-hreflang audit should surface
...
- For per-page hreflang validation, stay inside `autoseo-hreflang`.
- For broader scaled-content scoring (entropy of translated pages,
  AI-pattern detection in body), defer to `autoseo-content` via
  `<plugin-root>/scripts/autoseo run content_quality.py`.
