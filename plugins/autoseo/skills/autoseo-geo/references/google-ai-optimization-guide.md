...
Search ranking. Every autoseo audit that touches GEO should treat this doc
...
> autoseo's scores are heuristics, not Google-internal signals — state this
...
> [[llmstxt-evidence]] (Google Search representatives and published crawler
> documentation). Both files must stay aligned.
...
1. **Merchant Center — AI-generated product images:** must carry IPTC
   ...
   `skills/autoseo-images/SKILL.md` for the audit + injection pattern.
...
Full audit criteria: `skills/autoseo-technical/references/agent-friendly-pages.md`.
...
`skills/autoseo-ecommerce/references/ucp-universal-commerce-protocol.md`.

## How autoseo treats this guide

1. `autoseo-geo` audits cite this URL as the authoritative source whenever the
...
3. Where a third-party claim and Google contradict, autoseo defers to
   Google and notes the contradiction explicitly.
4. `autoseo-ecommerce` and `autoseo-images` enforce the two operational requirements
