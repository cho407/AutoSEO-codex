...
| Command | Purpose |
|---------|---------|
| `@autoseo sxo <url>` | Full SXO analysis (auto-detect keyword from page) |
| `@autoseo sxo <url> <keyword>` | Full SXO analysis for a specific keyword |
| `@autoseo sxo wireframe <url>` | Generate IST/SOLL wireframe with concrete placeholders |
| `@autoseo sxo personas <url>` | Persona-only scoring (skip SERP analysis) |

## Execution Pipeline

### Step 1: Target Acquisition

1. Fetch the target URL via `<plugin-root>/scripts/autoseo run render_page.py <URL> --mode auto` (SPA-aware and SSRF-safe)
2. Parse with `<plugin-root>/scripts/autoseo run parse_html.py <URL>` to extract: title, H1, meta description,
...
Only execute when `@autoseo sxo wireframe` is invoked.
...
| Finding | Hand Off To |
|---------|-------------|
| E-E-A-T gaps in persona scoring | `@autoseo content` for deep E-E-A-T audit |
| Missing schema types | `@autoseo schema` for generation |
| Local intent detected in SERP | `@autoseo local` for GBP analysis |
| Content depth gaps | `@autoseo page` for deep page analysis |
| Technical issues found during fetch | `@autoseo technical` for full audit |
| Image/media gaps | `@autoseo images` for optimization |
...
- [ ] Target URL was fetched via `<plugin-root>/scripts/autoseo run render_page.py <URL> --mode auto` (not raw curl/fetch)
